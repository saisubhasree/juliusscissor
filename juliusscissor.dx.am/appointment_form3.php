...
//Storing the values of appointment id and email id
$Appt_id = $_POST['appt_id'];
$Email_id = $_POST['email_id'];

//Perform inner join on appointments and customer table on customer id
...
//Update the appointment status of the appointment row of the customer to "Canceled"
while($row = mysqli_fetch_assoc($get_row)){
  if($Appt_id == $row['appt_id'] && (strcasecmp($Email_id, $row['email_id'])==0)) {

        $sql = "UPDATE appointments SET appt_status='Canceled' WHERE appt_id='$Appt_id'";
...
//Retrieve the appointment information of the customer such as customer first name, last name and timeslot option
while($row = mysqli_fetch_assoc($get_row)){
  if($Appt_id == $row['appt_id']){
    $cust_id = $row['customer_id'];
    $cust_fname = $row['customer_fname'];
    $cust_lname = $row['customer_lname'];
    $timeslot_id = $row['timeslot_id'];
    
//Switch case to retrieve the timeslot option from the timeslot id
    switch ($timeslot_id) {
    case "1":
...
//Error message to display when the appointment id or email id is not correct
$message ="";
if($error_flag==1){ 
...
<div class="w3-container w3-animate-left w3-card-4">
  ...
  <br>
  <center><h2>Appointment Cancellation</h2></center>
  <br>
  ...
  <?php if ($flag) { ?>
    <p class="mydiv">Your appointment is canceled</p>
  <?php } else { ?>
    <p class="myotherdiv">Sorry, your appointment could not be canceled</p>
  <?php } ?>
